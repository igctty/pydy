...
class MyMNIST(Chain):
    def __init__(self):
        super(MyMNIST, self).__init__(
            # L.Convolution2D(チャンネル数(一つの画像データにおける画像の枚数、RGBだと3), フィルター数（特徴の抽出を行うフィルタ数）, フィルタのサイズ（ピクセル数）)
            cnn1=L.Convolution2D(1, 15, 5), # 画像サイズ(1, 28, 28) → (15, 24, 24)
            cnn2=L.Convolution2D(15, 40, 5), # 画像サイズ(15, 12, 12) → (40, 8, 8)
            l1=L.Linear(640, 400), # 入力は40 x 4 x 4 = 640
            l2=L.Linear(400, 10),
        )
    ...
    def predict(self, x):
        ...
        h1 = F.max_pooling_2d(F.relu(self.cnn1(x)), 2)  # 画像サイズ(15, 24, 24) → (15, 12, 12)：cnn2のConvoへ。
        h2 = F.max_pooling_2d(F.relu(self.cnn2(h1)), 2)  # 画像サイズ(40, 8, 8) → (40, 4, 4)：L1の入力に。
        h3 = F.dropout(F.relu(self.l1(h2))) #中間層のドロップアウト
        return self.l2(h3)
...
# -- 学習 --
iterator = iterators.SerialIterator(train_data, 500) #バッチサイズ：500
